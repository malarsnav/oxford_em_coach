import { bootstrap, getSession, signIn, signOut, saveAttempt, updateTask, createProgramme, addAcademicResult, addJournalEntry, addReasoningSession, saveWeeklyReview, addInterviewSession } from './dataService.js';
...
function render() {
  ...
  app.innerHTML = `
    <aside class="sidebar">
      ...
      <nav>${navButton('dashboard','Dashboard')}${navButton('programme','Weekly Programme')}${navButton('tara','TARA Practice')}${navButton('analytics','TARA Analytics')}${navButton('academics','A-Level')}${navButton('journal','E&M Journal')}${navButton('reasoning','Oxford Reasoning')}${navButton('milestones','Milestones')}${navButton('interview','Interview Prep')}${navButton('review','Weekly Review')}</nav>
...
function dashboardHtml() {
  ...
  return `
    ...
    <section class="grid six">
      ...
      ${card('TARA Mastery', `${data.tara.overallAccuracy}% accuracy`, `Strongest: ${data.tara.strongestType?.name || 'Not enough data'}<br>Weakest: ${data.tara.weakestType?.name || 'Not enough data'}`)}
...
function taraHtml() {
  if (!state.practice) return `<header class="top"><div><p class="eyebrow">TARA Practice</p><h2>5-question methodology set</h2></div><button data-action="start-tara">Start 5-question set</button></header>`;
...
function card(title, metric, body) {
  return `<article class="panel card"><h3>${title}</h3>${metric ? `<p class="metric">${metric}</p>` : ''}<p>${body || ''}</p></article>`;
}
...
function startTara() {
  state.practice = { set: [...questions].sort(()=>Math.random()-0.5).slice(0,5), index: 0, answers: {}, startedAt: new Date().toISOString(), report: false };
  state.view = 'tara';
}
