...
// A small, reproducible EM monogram; no third-party artwork or font dependency.
const glyphs=['11111010001','10000011011','10000010101','11110010101','10000010001','10000010001','11111010001'];
for(const size of [180,192,512]){
  const scale=Math.floor(size/17),left=Math.floor((size-11*scale)/2),top=Math.floor((size-7*scale)/2);
  const pixels=Buffer.alloc(size*(1+size*3));
  for(let y=0;y<size;y++)for(let x=0;x<size;x++){
    const gx=Math.floor((x-left)/scale),gy=Math.floor((y-top)/scale);
    const white=gx>=0&&gx<11&&gy>=0&&gy<7&&glyphs[gy][gx]==='1';
