...
(async()=>{
  ...
  try {
    ...
    for (const width of [390,1440]) {
      ...
      assert.equal(await page.locator('[data-view="review"]').count(),0);
      assert.equal(await page.locator('.weekly-report').count(),0);
      await page.getByRole('button',{name:'View weekly report',exact:true}).click();
      assert.equal(await page.locator('.weekly-report .weekly-block').count()>7,true);
      assert.ok((await page.locator('.weekly-report h3').first().textContent()).includes('/16 blocks logged'));
      await page.getByText(/Partial week/).waitFor();
      assert.equal(await page.locator('.weekly-block').getByText('Tracking had not started',{exact:true}).count(),5);
      await page.getByRole('button',{name:'Previous week',exact:true}).click();
      await page.getByText('Tracking had not started. No missed targets for this week.',{exact:true}).waitFor();
      await page.getByRole('button',{name:'Next week',exact:true}).click();
      assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
      await page.screenshot({path:path.join(root,`../work/weekly-${width}.png`),fullPage:true});
      ...
      assert.equal(await page.locator('.tracker-block[open]').count(),1);
      await page.locator('.tracker-block').first().locator('[name=study_notes]').fill('Unsaved draft survives switching blocks');
      await page.locator('.tracker-block > summary').nth(1).click();
      assert.equal(await page.locator('.tracker-block[open]').count(),1);
      await page.locator('.tracker-block > summary').first().click();
      assert.equal(await page.locator('.tracker-block').first().locator('[name=study_notes]').inputValue(),'Unsaved draft survives switching blocks');
      await page.locator('.tracker-block > summary').first().click();
      await page.evaluate(()=>window.scrollTo(0,0));
      await page.screenshot({path:path.join(root,`../work/tracker-${width}.png`),fullPage:true});
      ...
      await log.evaluate(form=>{const block=form.closest('.tracker-block');if(block)block.open=true;});
      ...
      assert.equal(await log.locator('.selected-topic-details').getAttribute('open'),'');
      ...
      filter=page.locator('[data-action="plan-filter"]');await page.getByRole('button',{name:'By subject',exact:true}).click();await filter.locator('[name=subject]').selectOption('Maths');await filter.locator('[name=from]').fill('2026-09-01');await filter.locator('[name=to]').fill('2026-09-30');await filter.getByRole('button').click();
      assert.ok(await page.locator('[data-action="save-study-log"]').count()>4);
      assert.equal(await page.locator('[name=log_date]').first().inputValue(),'2026-09-05');
      assert.equal(await page.locator('[name=topics_covered]').first().inputValue(),'Quadratics');
      ...
      await smc.evaluate(form=>{const block=form.closest('.tracker-block');if(block)block.open=true;});
      ...
      await page.getByText('Timetable and coverage',{exact:true}).click();
      ...
      await block.evaluate(form=>{const block=form.closest('.tracker-block');if(block)block.open=true;});
      ...
      await changed.evaluate(form=>{const block=form.closest('.tracker-block');if(block)block.open=true;});
      ...
      await page.getByRole('heading',{name:'Tracking had not started',exact:true}).waitFor();
      ...
      await skipped.evaluate(form=>{const block=form.closest('.tracker-block');if(block)block.open=true;});
      ...
      await page.getByRole('button',{name:'Dashboard',exact:true}).click();
      await page.locator('.focus-card').getByText('4 followed plan · 1 changed · 1 skipped',{exact:true}).waitFor();
      ...
      await buffer.evaluate(form=>{const block=form.closest('.tracker-block');if(block)block.open=true;});
