...
(async()=>{
  ...
  try {
    ...
    for (const width of [390,1440]) {
      ...
      await page.getByRole('button',{name:'Today',exact:true}).click();
      assert.equal(await page.locator('.tracker-block').count(),7);
      const morning=page.locator('.tracker-block').nth(0);
      await morning.locator('summary').first().click();
      await morning.locator('[name=subject]').fill('History');
      await morning.locator('[name=work_done]').fill('Complete source analysis homework');
      await morning.getByRole('button',{name:'Save progress',exact:true}).click();
      await morning.getByText('Progress saved.',{exact:true}).waitFor();
      await page.reload();
      await page.getByRole('button',{name:'Plan Tracker',exact:true}).click();
      await page.locator('.tracker-block').nth(0).locator('summary').first().click();
      assert.equal(await page.locator('.tracker-block').nth(0).locator('[name=work_done]').inputValue(),'Complete source analysis homework');
      assert.equal(await page.locator('.tracker-block').nth(1).locator('[name=work_done]').inputValue(),'');
      assert.equal(await page.locator('.tracker-block').nth(2).locator('[data-rich-area="Maths"]').count(),1);
      assert.ok((await page.locator('.tracker-block').nth(2).locator('summary').first().textContent()).includes('Maths tuition'));
      assert.equal(await page.locator('.tracker-block').nth(3).locator('[data-rich-area="Maths"]').count(),1);
      assert.equal(await page.locator('.tracker-block').nth(4).locator('[data-rich-area="Magazine"]').count(),1);
      assert.equal(await page.locator('.tracker-block').nth(6).locator('[data-rich-area="EPQ"]').count(),1);
      await page.screenshot({path:path.join(root,`../work/weekday-plan-${width}.png`),fullPage:true});
      assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth),true);
      await page.getByRole('button',{name:'Dashboard',exact:true}).click();
      await page.getByText('1/7 planned study blocks logged today',{exact:true}).waitFor();
      await page.clock.setFixedTime(new Date('2026-09-26T12:00:00Z'));
      await page.getByRole('button',{name:'Today',exact:true}).click();
      await page.getByText('0/13 planned study blocks logged today',{exact:true}).waitFor();
      await page.getByRole('button',{name:'Plan Tracker',exact:true}).click();
      await page.getByRole('button',{name:'Today',exact:true}).click();
      assert.equal(await page.locator('.tracker-block').count(),13);
      const weekend=page.locator('.tracker-block').nth(1);
      await weekend.locator('summary').first().click();
      await weekend.locator('[name=subject]').fill('Physics');
      await weekend.locator('[name=work_done]').fill('Weekend homework completed');
      await weekend.getByRole('button',{name:'Save progress',exact:true}).click();
      await weekend.getByText('Progress saved.',{exact:true}).waitFor();
      await page.reload();
      await page.getByRole('button',{name:'Plan Tracker',exact:true}).click();
      await page.locator('.tracker-block').nth(1).locator('summary').first().click();
      assert.equal(await page.locator('.tracker-block').nth(1).locator('[name=work_done]').inputValue(),'Weekend homework completed');
      assert.equal(await page.locator('.tracker-block').nth(0).locator('[name=work_done]').inputValue(),'');
      assert.equal(await page.locator('.tracker-block').nth(2).locator('[name=work_done]').inputValue(),'');
      assert.equal(await page.locator('.tracker-block').nth(12).locator('[data-rich-area="EPQ"]').count(),1);
      await page.screenshot({path:path.join(root,`../work/weekend-plan-${width}.png`),fullPage:true});
      assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth),true);
      await page.getByRole('button',{name:'Dashboard',exact:true}).click();
